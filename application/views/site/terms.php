	<!-- CONTENT START -->
	<div id="red-content">
		<div id="red-terms-general">
			<ul>
				<li><h3>Perkenalan Kebijakan Layanan</h3></li>
				<li><p>Kebijakan Layanan digunakan untuk melindungi Rekan Netcoid, Tolong baca dengan teliti Hak dan Kewajiban Anda. karena dengan mengunakan layanan Netcoid ( www.networks.co.id ), Anda <strong>setuju</strong> untuk mengikuti beberapa ketentuan yang berada di bawah ini. Klarifikasi Kebijakan Layanan di help@networks.co.id. 
				</p></li>
				<hr/>
				<li><h3>1. Persetujuan</h3></li>
				<li><h4>1.1 Ketersediaan</h4><p>Layanan netcoid disediakan berdasarkan "Sebagaimana Adanya" dan " Sebagaimana Tersedia", netcoid berhak untuk memodifikasi, menhentikan sementara, atau menhentikan layanan, atas kebijakannya sendiri, setiap saat dan tampa ada pengumuman. Anda setuju bahwa netcoid tidak akan bertanggung jawab atas anda, terhadap modifikasi, penangguhan, atau pemberhentian layanan ini
				</p></li>
				<li><hr/><h4>1.2 Konten</h4><p>netcoid tidak bertanggung jawab atas segala konten, produk atau hal lainnya, yang diposting, dikirimkan
				dan dipublikasikan oleh pengguna netcoid. Anda bersedia dihubungi, di nonaktifkan atau digugat tampa pemberitahuan jika anda melanggar <a href="#">ketentuan produk</a> dan  <a href="#">ketentuan konten</a>. Jika anda melihat user, perusahaan atau organisasi yang melanggar ketentuan dan persyaratan konten, diharapkan untuk segera menghubungi 
				<a href="/help">Pusat Bantuan Konten</a>
				</p>
				<h4>1.2.1 Ketentuan Konten</h4>
					<ul>
						<li>Tidak boleh bermuatan SARA dan kesusilaan <a href="#">UU-ITE BAB VII Pasal 27 ayat 1</a><sup>3</sup></li>
						<li>Pengguna secara pribadi bertanggung jawab atas konten yang diterbitkan, <a href="#">UU-ITE BAB III Pasal 6</a><sup>1</sup></li>
						<li>Anda bersedia untuk mempublikasi hal - hal yang perlu publikasikan, <a href="#">UU-ITE BAB III Pasal 9</a><sup>2</sup></li>
						<li>Tidak boleh bermuatan JUDI berlaku untuk produk, <a href="#">UU-ITE BAB VII Pasal 27 ayat 2</a><sup>4</sup></li>
						<li>Konten selain dapat dipertanggung jawabkan(2), harus bersifat asli atau tidak bohong, menyesatkan lalu merugikan konsumen.</li>
						<li>Tidak boleh bermuatan ANCAMAN, <a href="#">UU-ITE BAB VII Pasal 29</a><sup>5</sup></li>
						<li>Konten tidak boleh bermuatan Dokumen palsu dengan memanipulasi, perubahan, menciptakan, pinghilangan atau pengerusakan Dokumen atau Informasi Elektronik agar seolah - olah dianggap asli. contoh: partnership, disponsori dan sebagainya</li>
					</ul>
				<h4>1.2.2 Ketentuan Produk</h4>
					<ul>
						<li>Produk tidak boleh melanggar ketentuan merek terdaftar, jika anda menjual produk merek terdaftar maka hukum yang berlaku adalah sesuai dengan merek terdaftar yang dijual yang dikeluarkan oleh kebijakan pemilik merek</li>
						<li>Tidak boleh bermuatan JUDI berlaku untuk Konten, <a href="#">UU-ITE BAB VII Pasal 27 ayat 2</a><sup>6</sup></li>
						<li>PORNOGRAFI, NARKOBA, DAN SEGALA MACAM PRODUK YANG TELAH DILARANG OLEH PEMERINTAH REPUBLIK INDONESIA<sup>7</sup></li>
					</ul>
				</li>
				<h4>1.2.3 Ketentuan Tambahan</h4>
					<ul>
						<li>netcoid tidak bertanggung jawab atas informasi yang kami gunakan dari pihak ketiga atau partner kami.</li>
						<li>netcoid tidak memastikan keakurasian nilai value tukar yang berada pada layanan kami</li>
					</ul>
				<li><hr/><h4>1.3 Hak Cipta &amp; Merek</h4><p>netcoid adalah, kecuali dinyatakan lain, pemilik semua hak cipta dan data dalam layanan dan isinya. individu, perusahaan atau organisasi tertentu yang memposting produk, gambar, dan konten adalah hak cipta dari pemilik, agen atau karyawan yang diperbolehkan
				oleh hukum. Anda tak boleh memperbanyak, mendistribusi, mempublikasi atau melakukan, tampa ada persetujuan dari pemilik tersebut. netcoid tidak
				mengklaim hak kepemilikan dalam produk - produk anda. Anda setuju bahwa anda tidak akan mendistribusi selain yang diizinkan oleh pemilik produk,
				gambar, dan konten. Jika anda memlihat user, perusahaan atau organisasi yang melanggar Hak Cipta atau Merek anda, silahkan menghubungi 
				<a href="#">Pusat Bantuan Hak Cipta</a>
				</p></li>
				<li><hr/><h4>1.4 Transaksi</h4><p>netcoid memberikan layanan media penhubung antar usaha tampa ada pihak ketiga, netcoid tidak bertanggung jawab atas segala penipuan yang dilakukan oleh user lain atau transaksi yang dilakukan pihak ketiga.
				</p>
				<h4>1.4.1 Tips &amp; Triks Bertransaksi</h4>
					<ul>
						<li>Periksa apakah ada tanda verifikasi terdapat pada profile perusahaan, jika terdapat silahkan melihat dokumen - dokumen
						perusahaan tersebut, selalu periksa kembali apakah surat tersebut masih sah, atau terdaftar di badan pemerintah
						</li>
						<li>COD, Cash On Delivery, utamakan perusahaan yang mengunakan sistim tranksaksi pada saat bertemu</li>
					</ul>
				</li>
			</ul>
		</div>

		<div id="red-terms-general">
			<ul>
				<li><h3>2. Keamanan</h3></li>
				<li>Lihat bagian <a href="/privacy#keamanan">Kebijakan Privasi #Keamanan</a></li>
			</ul>
		</div>

		<div id="red-terms-general">
			<ul>
				<li><h3>3. Akun</h3></li>
				<li><h4>3.1 Managemen</h4><p>Layanan netcoid berhak untuk menonaktifkan akun tampa pemberitahuan dikarenakan beberapa hal sebagai berikut:</p>
					<ul>
						<li>Pelanggaran Persetujuan</li>
						<li>Melakukan aktifitas yang mencurigakan</li>
						<li>Tidak aktif selama waktu yang telah ditentukan</li>
					<ul>
				</li>
			</ul>
		</div>
		
		<div id="red-terms-notes">
			<ul>
				<li><h3>Notes</h3></li>
				<li id="note-terms-8">8. untuk kerjasama kami akan memberikan satu account hosting sandbox ( untuk percobaan / latihan ) + account hosting untuk forum website komunitas, kententuan berlaku.</li>
				<li id="note-terms-9">9. One way hashing, kami tidak menyimpan password & email anda secara langsung ( tidak dalam bentuk text )</li> 
			</ul>
		</div>
	</div>
	<!-- CONTENT END -->	
