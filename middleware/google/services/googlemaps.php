...
class googlemaps {
	...
	function show() {
		...
		$target = curl_init ( 'http://maps.google.com/maps/api/staticmap?center=' . urlencode ( $this->address ) . '&zoom=14&size=' . $this->size . '&sensor=false&markers=icon:http://chart.apis.google.com/chart%3Fchst%3Dd_map_spin%26chld%3D1%257C0%257Cfff%257C11%257C_%257CHere|' . urlencode ( $this->address ) );
		...
		if ($header_size == 256) {
			...
			$url = 'http://maps.google.com/maps/api/staticmap?center=' . urlencode ( $this->address ) . '&zoom=16&size=' . $this->size . '&sensor=false&markers=icon:http://chart.apis.google.com/chart%3Fchst%3Dd_map_spin%26chld%3D1%257C0%257Cfff%257C11%257C_%257C' . $this->language . '|' . urlencode ( $this->address );
