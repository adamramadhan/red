...
class xhttp_cookie {
	...
	public static function export_cookies() {
		return self::$datastore;
	}

	public static function import_cookies($cookies, $mode = self::IMPORT_MERGE) {
		if($mode == self::IMPORT_REPLACE) {
			self::$datastore = $cookies;
		} elseif($mode == self::IMPORT_ADD) {
			self::$datastore = self::$datastore + $cookies;
		} else {
			array_merge(self::$datastore, $cookies);
		}
	}
}
