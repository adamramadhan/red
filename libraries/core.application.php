...
class Application {
	...
	/**
	 * renders css, js, twig, or later etc.
	 * @param string $vendor
	 * @param string $package
	 * @author rama@networks.co.id
	 * @tutorial wiki/missing.txt
	 * @todo !EXPERIMENTAL $this->render('jquery','js'); $this->render('twigtemplate,'twig')
	 * nantinya views akan digantikan oleh render
	 */
	protected function render($how, $what, $options = array()){
		...
		switch ($how) {
			...
			case 'js':
				...
				if (!empty($options['type'])) {
					...
					# LOAD RAW
					if ($options['type'] == 'raw') {
						echo '<script type="text/javascript">'. $what .'</script>';
					}
				}
